...
- 📦 **Issue JSON Mode (Gemma)**: Generate a structured JSON issue report from a screenshot
- 🧩 **Structured Reasoning Mode (OpenAI)**: Clean, sectioned markdown answers for clarity
...
npm run server:openai  # Start with OpenAI GPT OSS 20B (excellent reasoning)
...
npm run openai -- -p "Explain quantum computing in simple terms"
...
npm run server:openai  # Start server with OpenAI GPT OSS 20B model
...
npm run openai         # Use OpenAI GPT OSS 20B model (excellent reasoning, no images)
npm run gemma          # Use Gemma model (supports images)
npm run unload         # Unload current model
npm run load:openai    # Load OpenAI GPT OSS 20B model
npm run load:gemma     # Load Gemma model
npm run help           # Show help and usage information
npm run issue:json     # Example: issue JSON generation with Gemma
```
...
Options:
  ...
  -f, --file <path>      Path to a text file (repeat flag or use comma-separated list for multiple)
...
npm run client -- -m openai/gpt-oss-20b -p "Explain quantum computing"
...
npm run client -- -m openai/gpt-oss-20b -f document.txt -p "Summarize this"

# Combined analysis
npm run client -- -m google/gemma-3-12b -i screenshot.png -f context.txt -p "Analyze with context"

# Multiple files
npm run client -- -m openai/gpt-oss-20b -f notes.md -f spec.txt -p "Summarize"
npm run client -- -m openai/gpt-oss-20b -f notes.md,spec.txt -p "Summarize"

# Save output to markdown file
npm run client -- -i image.png -p "What do you see?" -o outputs/analysis.md

# Issue JSON from a screenshot (Gemma)
npm run client -- -m google/gemma-3-12b -i images/image.png -J -O outputs/issue.json

# Structured, clear reasoning output (OpenAI)
npm run client -- -m openai/gpt-oss-20b -f outputs/issue.json -R
```
...
curl -X POST http://localhost:3000 -H "Content-Type: application/json" \
  -d '{"action": "load", "model": "openai/gpt-oss-20b"}'

# Unload current model
curl -X POST http://localhost:3000 -H "Content-Type: application/json" \
  -d '{"action": "unload"}'

# Process a request (multiple files)
curl -X POST http://localhost:3000 -H "Content-Type: application/json" \
  -d '{
    "model": "openai/gpt-oss-20b",
    "prompt": "Summarize",
    "files": ["/abs/path/notes.md", "/abs/path/spec.txt"]
  }'
  -d '{"model": "openai/gpt-oss-20b", "prompt": "Hello", "file": "document.txt"}'
```
...
When using `--issue-json`, the server asks Gemma to return JSON only with a concise schema. The client:

- Prints the parsed JSON to stdout if possible
- Saves to the path given by `--json-output`

Example JSON structure:

```json
{
  "title": "Error dialog: Authentication failed",
  "summary": "Login form shows an 'authentication failed' dialog after submit.",
  "question": null,
  "answer_options": ["Retry", "Reset password", "Cancel"],
  ...
  "extracted_text": ["Authentication failed", "Error code: 401"],
  "probable_root_cause": "Invalid credentials or expired session token",
  "suggested_next_prompt": "Given the screenshot of an auth error, outline steps to diagnose 401s in a web app with React + Node backend.",
  "confidence": 0.78
}
```
...
- **Document Analysis**: Use OpenAI GPT OSS 20B for summarizing, reviewing, and analyzing text documents
- **Code Review**: Use OpenAI GPT OSS 20B for analyzing code files and suggesting improvements
- **Text-Only Conversations**: Use OpenAI GPT OSS 20B for general conversations and complex reasoning
- **Combined Analysis**: Use Gemma with images + OpenAI GPT OSS 20B for text analysis
- **Content Creation**: Use OpenAI GPT OSS 20B for generating content based on file inputs
- **Problem Solving**: Use OpenAI GPT OSS 20B for solving math problems, puzzles, or logic questions
...
6. **Model Validation Error**: Use the correct model for your task (OpenAI GPT OSS 20B for text, Gemma for images)
